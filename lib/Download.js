(function(µ,SMOD,GMOD,HMOD,SC){
	...
	let DOWNLOAD=µ.Class(DBObj,{
		...
		constructor:function(param)
		{
			...
			this.originalField=param.originalField||null;
		...
		getSpeed:function()
		{
			if(!this.startTime||!this.time||this.startTime===this.time) return 0;
			return (this.size - this.startSize)/((this.time - this.startTime)/1000);
		},
		getCurrentSpeed:function()
		{
			if(!this.lastTime||!this.time) return 0;
		...
		equals(other)
		{
			...
			return !(
				other == null ||
				this.ID !== other.ID ||
				JSON.stringify(this.dataSource) !== JSON.stringify(other.dataSource)
			);
		},
		//compatibility
		getItems:function()
	...
	DOWNLOAD.parseFilesize=function(filesize)
	{
		...
		{
			return SC.unit.from(filesize)||0;
		}
		return filesize;
	};
	// TODO remove
